...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .header-text .est   { font-size: 7.5pt; font-weight: bold; text-transform: uppercase; }
    ...
    .doc-title h2 {
        ...
        letter-spacing: 3px; border-bottom: 2px solid #000;
...
<div class="header">
    <div class="header-inner">
        ...
        <div class="header-text">
            ...
            <p class="est">Estado Bolivariano de {{ $institucion->estado }}</p>
            @endif
...
<div class="cuerpo">
    ...
    <p>
        ...
        nacido(a) en fecha
        <span class="enfasis">{{ \Carbon\Carbon::parse($estudiante->fecha_nacimiento)->locale('es')->isoFormat('D [de] MMMM [de] YYYY') }}</span>
        ...
        @if($estudiante->estado_nacimiento && $estudiante->lugar_nacimiento), @endif
        ...
        <span class="enfasis">{{ $anio->codigo_ano_escolar }}</span>,
        en el turno <span class="enfasis">{{ ucfirst($seccion->turno ?? '') }}</span>.
    </p>
    @if(isset($matricula->representante) && $matricula->representante)
    <br>
    <p>
        Su representante legal es:
        <span class="enfasis">{{ strtoupper($matricula->representante->nombres) }} {{ strtoupper($matricula->representante->apellidos) }}</span>,
        titular de la Cédula de Identidad Nro. <span class="enfasis">{{ $matricula->representante->cedula_representante }}</span>,
        en calidad de <span class="enfasis">{{ $matricula->representante->parentesco ?? 'Representante' }}</span>.
    </p>
    @endif
    <br>
    <p>
